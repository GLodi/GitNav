...
class RepoAboutFragment : BaseFragment(), RepoAboutContract.View {
    ...
    override fun populateGridView(numberList: MutableList<String>) {
        ...
        (repo_about_fragment_gridview.adapter as RepoAboutAdapter).getIssuesClick()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe { _ ->
                    startActivity(IssueListActivity.getIntent(context).putExtra("owner", mOwner).putExtra("name", mName))
                    activity.overridePendingTransition(0,0)
                }
    }
