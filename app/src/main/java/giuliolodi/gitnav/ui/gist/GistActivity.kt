...
class GistActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var gistFragment: GistFragment? = supportFragmentManager.findFragmentByTag(GIST_FRAGMENT_TAG) as GistFragment?
        if (gistFragment == null) {
            gistFragment = GistFragment()
            supportFragmentManager
                    .beginTransaction()
                    .replace(R.id.gist_activity_frame, gistFragment, GIST_FRAGMENT_TAG)
                    .commit()
        }
    }
