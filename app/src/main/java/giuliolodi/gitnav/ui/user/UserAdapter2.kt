...
import kotlinx.android.synthetic.main.row_user2.view.*
import org.eclipse.egit.github.core.User

/**
 * Created by giulio on 12/06/2017.
 */

class UserAdapter2 : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class UserHolder(root: View) : RecyclerView.ViewHolder(root) {
        fun bind (user: User) = with(itemView) {
            user.login?.let { row_user2_username.visibility = View.GONE }
            Picasso.with(context).load(user.avatarUrl).resize(100,100).centerCrop().into(row_user2_image)
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == 1) {
            val view = (LayoutInflater.from(parent?.context).inflate(R.layout.row_user2, parent, false))
